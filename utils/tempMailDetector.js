...
/**
 * Extracts and cleans the domain from an email address or domain string.
 * Splits using '@' and grabs the host part.
 * 
...
function extractDomain(emailOrDomain) {
    ...
    // Take the domain part if email was provided, or full string if domain was provided
...
/**
 * Checks whether an email address or domain is a temporary/disposable email.
 * 1. Checks default whitelist (gmail.com, outlook.com, yahoo.com, icloud.com).
 * 2. Checks MongoDB database for cached domain check.
 * 3. If not found in DB, queries TempMailDetector API.
 * 4. Saves domain check result to MongoDB.
 * 5. Blocks response if score >= 75 or block_list is true.
 * 
 * @param {string} emailOrDomain 
...
async function checkDomain(emailOrDomain) {
    ...
    // 2. Check MongoDB for existing record
    try {
        ...
        // Continue to API check if DB read fails
    }

    // 3. Call TempMailDetector API
    try {
        ...
        // Check special error responses if returned in data
        if (data.error || data.message) {
            ...
            if (errStr === 'no_lookups_remaining') {
                console.error('[TempMailDetector] API Error: No lookups remaining on API key.');
            }
        }
        ...
        // Blocking criteria: score >= 75 OR block_list === true
        ...
        if (apiError.response) {
            ...
            if (status === 401) {
                console.error('[TempMailDetector] Unauthorized: Invalid or missing API key.');
            } else if (status === 402) {
                console.error('[TempMailDetector] Payment Required: Lookups quota exhausted.');
            }
        } else {
            console.error(`[TempMailDetector] Request error for ${domain}:`, apiError.message);
        }

        // Default fallback on error (allow or fail open/closed depending on policy)
        return {
            domain,
            isDisposable: false,
            blocked: false,
...
/**
 * Express Middleware to block registration/requests using disposable emails.
 * Expects `req.body.email` or custom parameter.
 */
function validateTempEmailMiddleware(emailField = 'email') {
    return async (req, res, next) => {
        try {
            ...
            // Attach result to req for downstream usage
